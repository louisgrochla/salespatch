...
export async function GET(req: NextRequest) {
  ...
  if (cat && ["infrastructure", "compute", "tools", "misc"].includes(cat)) {
    where.category = cat;
  }

  const rows = await prisma.costEntry.findMany({ where, orderBy: { date: "desc" } });
  ...
  const filename = `nerve-costs-${stamp}.${format}`;
  ...
  const cols: { key: keyof (typeof flat)[number] & string }[] = [
    { key: "id" }, { key: "date" }, { key: "category" }, { key: "amount" },
