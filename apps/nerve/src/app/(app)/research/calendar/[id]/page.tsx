  ...
  return (
    <div className="p-6">
      ...
      <PageHeader
        ...
        actions={
          editing ? (
            <HeaderLink href={`/research/calendar/${item.id}`}>cancel edit</HeaderLink>
          ) : (
            <>
              ...
              <HeaderLink href={`/research/calendar/${item.id}?edit=1`}>edit</HeaderLink>
              <Link href="/research/calendar" className="font-mono text-2xs uppercase tracking-wider text-fg-muted hover:text-fg border border-border hover:border-border-strong px-2 py-1">
                back
              </Link>
            </>
          )
      ...
      {editing ? (
        <CalendarForm action={updateAction} sections={sections} cancelHref={`/research/calendar/${item.id}`} submitLabel="Save changes"
        ...
        <div className="border border-border bg-bg-panel divide-y divide-border max-w-3xl">
          ...
          <Row label="linked section">
            {item.dissertationSection
              ? <Link href={`/research/sections/${item.dissertationSection.id}`} className="text-accent underline">{item.dissertationSection.chapter}</Link>
              : "—"}
          </Row>
        </div>
      )}
