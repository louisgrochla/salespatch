      ...
      <PageHeader
        ...
        actions={
          <>
            ...
            <HeaderPrimary href="/dissertation/evidence/new">+ new evidence</HeaderPrimary>
          </>
        }
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[14rem_1fr] gap-4">
        <aside className="space-y-4">
          <div className="border border-border bg-bg-panel">
            ...
            <ul>
              <li>
                <Link href="/dissertation/evidence" className="block px-3 py-1.5 font-mono text-xs hover:bg-bg-hover">
                  all
                </Link>
              </li>
              {sourceTypes.map((s) => (
                <li key={s.sourceType}>
                  <Link href={`/dissertation/evidence?sourceType=${s.sourceType}`}
          ...
          <div className="border border-border bg-bg-panel">
            ...
            <ul>
              ...
              {sections.map((s) => (
                <li key={s.id}>
                  <Link href={`/dissertation/evidence?sectionId=${s.id}`}
                    className="block px-3 py-1.5 font-mono text-xs hover:bg-bg-hover text-fg-muted">
                    {s.chapter}
                  </Link>
                </li>
          ...
          {items.length === 0 ? (
            ...
            <div className="border border-border bg-bg-panel divide-y divide-border">
              {items.map((e, i) => {
                ...
                return (
                  <Link key={e.id} href={`/dissertation/evidence/${e.id}`} className="block px-4 py-3 hover:bg-bg-hover">
