  ...
  return (
    <div className="p-6">
      ...
      <PageHeader
        ...
        actions={
          editing ? (
            <HeaderLink href={`/dissertation/evidence/${e.id}`}>cancel edit</HeaderLink>
          ) : (
            <>
              ...
              <HeaderLink href={`/dissertation/evidence/${e.id}?edit=1`}>edit</HeaderLink>
              <Link href="/dissertation/evidence" className="font-mono text-2xs uppercase tracking-wider text-fg-muted hover:text-fg border border-border hover:border-border-strong px-2 py-1">
                back
              </Link>
            </>
          )
      ...
      {editing ? (
        <EvidenceForm
          action={updateAction}
          sections={sections}
          cancelHref={`/dissertation/evidence/${e.id}`}
        ...
        <div className="border border-border bg-bg-panel divide-y divide-border max-w-3xl">
          ...
          <div className="px-4 py-3">
            ...
            {e.dissertationSection
              ? <Link href={`/dissertation/sections/${e.dissertationSection.id}`} className="text-accent underline">{e.dissertationSection.chapter}</Link>
