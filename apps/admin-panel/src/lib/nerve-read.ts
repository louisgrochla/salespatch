...
// canonical-query-string signing scheme in get-ingest.sh and the route
// handlers in apps/nerve/src/app/api/read/*.
//
// Caller passes the endpoint path (eg "/api/read/pending-assignments")
...
// between thrown errors and non-2xx responses. Fire-and-forget by
// design — if NERVE is down, the caller decides whether to degrade or
...
export async function nerveGet<T = unknown>(
  ...
  if (!secret) {
    return {
      ...
      status: 503,
      data: null,
      error: 'OUTCOME_INGEST_SECRET not configured in admin-panel env',
    };
  }
